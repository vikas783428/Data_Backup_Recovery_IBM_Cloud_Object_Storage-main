...
# Load credentials for Backup Server from the JSON file
backup_server_cos_credentials = load_cos_credentials("backup_server_credentials.json")

if not backup_server_cos_credentials:
    print("Error loading credentials for Backup Server. Exiting.")
    exit(1)
...
# Function to upload a file to Backup Server's bucket
def upload_file_to_backup_server(file_name):
    try:
        print(f"Uploading: {file_name}")
        with open(file_name, 'rb') as file_data:
            backup_server_cos.upload_fileobj(file_data, backup_server_cos_credentials['bucket_name'], file_name)
        print(f"Successfully uploaded: {file_name}")
    except Exception as e:
        print(f"Error uploading {file_name} to backup server: {str(e)}")

# Function to upload an entire directory
def upload_directory_to_backup_server(directory_path):
    try:
        for root, dirs, files in os.walk(directory_path):
            for file in files:
                file_path = os.path.join(root, file)
                upload_file_to_backup_server(file_path)
    except Exception as e:
...
class MyHandler(FileSystemEventHandler):
    def on_modified(self, event):
        if event.is_directory:
            ...
            upload_directory_to_backup_server(event.src_path)
            return
        ...
        upload_timers[file_path] = Timer(debounce_delay, upload_file_to_backup_server, [file_path])
    ...
    def on_created(self, event):
        ...
        upload_file_to_backup_server(event.src_path)
...
print("Error loading credentials for Main Server. Shifting to Backup Server.")
